...
\begin{figure}
	\centering
	\missingfigure{"Picture of a Phillips Hue Lightbulb"}
...
When dealing with new, unseen users and environments, gesture classification accuracy degrades significantly. \todo{citation needed}
...
If we are to take advantage of these new technologies, it is imperative that its practical applications be researched. \todo{I don't like the wording here}
...
Domain-agnostic systems are required for many industry applications, such as [what]. \todo{Figure out what}
...
The use of Wi-Fi sensing to detect gestures is one step closer to this futuristic dream and in addition to all the potential commercial and practical benefits this technology could bring, the sheer ``coolness'' of the technology should not be underestimated as a motivation to perform research. \todo{A bit less academic, but hopefully just as viable as a motivation}
...
These models are influenced by ``domain factors'' such as the subject performing the gesture and the environment where this is taking place.\todo{Replace "this is taking place" with something else}
...
Due to these shifts, performance of the model is degraded. \todo{citation needed}
...
In the case of Tesla, for example, having a large fleet of vehicles capable of recording what is essentially supervised training data makes it possible to collect the vast amounts of data required to build a dataset usable for the training of self-driving vehicles.\todo{specific citation needed}
Various OpenAI projects, on the other hand, simply scrape large amounts of websites and collect these as part of their dataset.\todo{specific citation needed}
...
\begin{itemize}
	...
	\begin{itemize}
		...
		\item Adversarial training procedure is then used in a min-max game \todo{citation needed}
	\end{itemize}
